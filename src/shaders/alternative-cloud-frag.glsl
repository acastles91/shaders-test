...
void main() {
   ...
   // vec2 curveZ[4] = vec2[](vec2(-0.6272, 0.0), vec2(-0.6272, 0.33), vec2(0.3375, 0.67), vec2(0.3375, 1.0));
    ...
    vec3 mixOverlay3 = mixOverlay(mixOverlay2, distanceVoronoi3, uFactor_3);
...
//WEB GL 2.0
    ...
    //fragColor = vec4(vec3(v_Displacement), 1.0);
}


//////TEST
////
//////gl_FragColor = vec4(curvedCoordinates, 1.0);
////
////    // Output the final volume result to the Material Output
//////    gl_FragColor = vec4(volume.color, 1.0);
////
//////    // Calculate the normal
//////    vec3 normal = normalize(cross(dFdx(v_ObjectSpacePosition), dFdy(v_ObjectSpacePosition)));
//////
//////    // Calculate the light direction
//////    vec3 lightDirection = normalize(lightPosition - v_ObjectSpacePosition);
//////
//////    // Compute the Lambertian diffuse shading
//////    float diffuse = max(dot(normal, lightDirection), 0.0);
//////
//////    // Apply the light color and intensity
//////    vec3 finalColor = gl_FragColor.rgb * lightColor * diffuse;
//////
//////    gl_FragColor = vec4(finalColor, 1.0); // Assuming fully opaque
////
////}
//
...
//
//void main() {
//    vec3 color = 0.5 * (v_ObjectSpacePosition + 1.0);
